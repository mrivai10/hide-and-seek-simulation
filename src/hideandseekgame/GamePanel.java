...
public class GamePanel extends JPanel implements ActionListener{
    ...
    private void init(){
        ...
        // Mengatur tata letak (layout) komponen secara manual
        setLayout(null);
        ...
        // ChangeListener ini akan merespons perubahan nilai pada JSlider.
        greenDroidSightSlider.addChangeListener(new ChangeListener() {
            public void stateChanged(ChangeEvent e) {
                JSlider source = (JSlider) e.getSource();
                // Memastikan bahwa aksi hanya dilakukan ketika pengguna telah selesai menggeser JSlider.
    ...
    private void randomizeMap() {
        // Setel ulang labirin ke semua dinding
        for (int i = 0; i < maze.length; i++) {
            // Mengatur seluruh elemen dalam maze menjadi 1, yang berarti mengatur seluruh sel sebagai tembok
            Arrays.fill(maze[i], 1);
        }

        // Penciptaan labirin dimulai dari pojok kiri atas
        // 0 untuk menunjukkan bahwa sel tersebut telah dikunjungi
        generateMazeRecursive(0, 0);

        // Buka pintu masuk dan keluar labirin
        maze[0][0] = 0;
    ...
    private void generateMazeRecursive(int row, int col) {
        ...
        for (int[] direction : directions) {
            ...
            if (newRow >= 0 && newRow < maze.length && newCol >= 0 && newCol < maze[0].length && maze[newRow][newCol] == 1) {
                // Hancurkan dinding antara sel saat ini dan sel berikutnya
    ...
    private void shuffleArray(int[][] array) {
        ...
        // Mengacak urutan elemen dalam array directions
        for (int i = array.length - 1; i > 0; i--) {
            int index = random.nextInt(i + 1);
    
            // Tukar elemen pada indeks dan i
    ...
    private void moveRedDroid() {
        ...
        if (redX == greenX && redY == greenY) {
            gameOver = true;
            resetRedDroids = true;
            return; // Hentikan pergerakan jika posisi kedua droid sama
        }
        ...
        if (!canSeeGreenDroid) {
            int[][] directions = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}}; // kanan - kiri - bawah - atas
        ...
        int[][] distance = new int[row][col]; //jarak dari droid merah ke setiap sel pada peta.
        boolean[][] visited = new boolean[row][col]; //menandai apakah suatu sel telah dikunjungi 
        int[][] parentX = new int[row][col]; // menyimpan koordinat parent dari setiap sel dalam jalur pencarian.
        int[][] parentY = new int[row][col];
        Queue<int[]> queue = new LinkedList<>(); // melacak sel yang akan dieksplorasi.

        // Inisialisasi array distance, visited, and parent
        for (int i = 0; i < row; i++) {
        ...
        distance[redX][redY] = 0; // jarak 0 dari dirinya sendiri.
        visited[redX][redY] = true; // menandai sel tersebut sebagai dikunjungi.
        queue.offer(new int[]{redX, redY}); // memasukkan koordinat droid merah (redX dan redY) ke dalam antrian (queue)

        int[][] directions = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

        while (!queue.isEmpty()) {
            // Selama antrian tidak kosong, ambil sel pertama dari antrian
            ...
            for (int[] direction : directions) {
                ...
                if (newX >= 0 && newX < row && newY >= 0 && newY < col && maze[newX][newY] != 1 && !visited[newX][newY]) {
                    ...
                    // Stop the BFS if greenDroid is found
        ...
        // Ikuti pointer parent untuk menemukan langkah selanjutnya untuk redDroid
        int nextX = greenX;
        int nextY = greenY;

        //hingga ditemukan posisi selanjutnya yang merupakan parent langsung dari posisi saat ini. 
    ...
    private void moveGreenDroid() {
        ...
        boolean[][] visited = new boolean[row][col];// melacak sel-sel yang telah dikunjungi
        
        // Terapkan algoritma DFS (Depth First Search) kepada droid hijau untuk menghindari droid merah dengan menelusuri peta
    ...
    private boolean dfs(int x, int y, boolean[][] visited, int redX, int redY, int sightRadius) {
        ...
        for (int[] direction : directions) {
            ...
            if (newX >= 0 && newX < row && newY >= 0 && newY < col && maze[newX][newY] != 1 && !visited[newX][newY] && (newX != redX || newY != redY)) {
                ...
                if (distanceToRedDroid > sightRadius) { // Cek apakah jarak dengan Droid Merah lebih besar dari jarak pandang
                    greenDroid.setPosition(newX, newY);
                    return true;
                }else if (distanceToRedDroid == sightRadius) { // Jika jarak sama dengan jarak pandang, droid hijau terjebak
                    return false;
                }
            }
        }
    
        // Jika semua sudah tetangga dikunjungi atau diblokir, kembali ke posisi sebelumnya
        for (int[] direction : directions) {
            int newX = x + direction[0];
            int newY = y + direction[1];
    ...
    public void paintComponent(Graphics g) {
        ...
        if(showGreenDroidView){
            // Tampilkan pandangan Droid hijau terhadap Droid merah
            int greenX = greenDroid.getX();  // Koordinat X Droid hijau
            int greenY = greenDroid.getY();  // Koordinat Y Droid hijau
            ...
            for (int i = -greenDroidSightRadius; i <= greenDroidSightRadius; i++) {
                for (int j = -greenDroidSightRadius; j <= greenDroidSightRadius; j++) {
                    ...
                    // Pastikan koordinat yang akan digambar berada dalam batas peta
                    if (drawX >= 0 && drawX < row && drawY >= 0 && drawY < col) {
                        g.setColor(new Color(0, 255, 0, 100));  // Warna hijau dengan kecerahan 100 (transparan)
